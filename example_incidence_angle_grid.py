...
from landschaftsgradient import InzidenWinkel, lv95_to_wgs84, parse_datetime
...
def logparameter(args, cfg):
    lon_loc, lat_loc = lv95_to_wgs84(args["east"], args["north"])
    ...
    logging.info(f"Grid:       Grid size={args["grid_size"]:.2f} m / Grid step={args["grid_step"]:.2f} m")
...
if __name__ == "__main__":
    ...
    if os.path.isdir(__cfg["logfolder_path"]):
        try:
            ...
            logger, _logfile = setup_logging(level=loglvl, logfolder=Path(__cfg["logfolder_path"]))
            run(__args, __cfg)
        except Exception:
            sys.exit(1)
    else:
