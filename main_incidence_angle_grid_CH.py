...
def run(args, cfg, coord_tuple):
    ...
    try:
        ...
        # calculate incidence for every 10x10m cell of 20x20km tile
            tile, transform = iw.calc_incidence_grid(
                e_lv95=coord_tuple[0],
                n_lv95=coord_tuple[1],
                dateoi=args["date"],
                timeoi=t_str,  # vary time here
                grid_size=args["grid_size"],
                grid_step=args["grid_step"],
            )
            ...
            if transform_ref is None:
                ...
                shape_ref = tile.shape
            else:
                if transform != transform_ref:
                    logging.warning("Transform changed across time slices; using the first transform.")
                if tile.shape != shape_ref:
                    raise ValueError(
                        f"Inconsistent tile shapes across time slices: got {tile.shape}, expected {shape_ref}"
                    )
        ...
        crs_value = getattr(iw, "crs", None) or "EPSG:2056"
